...
pair_days<-readLines(
  if(off_site){
    "https://raw.githubusercontent.com/satrapade/pairs/master/data/duke_pair_days.csv"
  } else {
    "N:/Depts/Share/UK Alpha Team/Analytics/duke_summary/duke_pair_days.csv"
  }
) %>% table2matrix

pair_local_pnl<-readLines(
  if(off_site){
...
drop_one_pair_pnl<-readLines(
  if(off_site){
    "https://raw.githubusercontent.com/satrapade/pairs/master/data/duke_drop_one_pair_pnl.csv"
  } else {
    "N:/Depts/Share/UK Alpha Team/Analytics/duke_summary/duke_drop_one_pair_pnl.csv"
  }
) %>% table2matrix

manager_exposure<-readLines(
  if(off_site){
...
drop_one_manager_pnl<-readLines(
  if(off_site){
...
manager_drop_one_pair_pnl<-readLines(
  if(off_site){
    "https://raw.githubusercontent.com/satrapade/pairs/master/data/duke_manager_drop_one_pair_pnl.csv"
  } else {
    "N:/Depts/Share/UK Alpha Team/Analytics/duke_summary/duke_manager_drop_one_pair_pnl.csv"
  }
) %>% table2matrix
...
duke_lt <- (if(off_site){
  "https://raw.githubusercontent.com/satrapade/pairs/master/data/duke_manager_look_vs_outright.csv"
} else {
...
log_code(z<-intraday_managers*10000)
colnames(z)[1]<-"DUKE"
log_code(z<-z[,colnames(duke_lt_matrix)])
log_code(x<-duke_lt_matrix[grepl("Index$",rownames(duke_lt_matrix)),])
log_code(y<-intraday_sector_indices[,rownames(x)]%*%x)
log_code(w<-factor_local_tret[,rownames(x)]%*%x)
log_code(v<-manager_local_pnl%>%{cbind(.,DUKE=rowSums(.))[,colnames(y)]})

log_code(explain<-data.table(
  pm=structure(colnames(y),hrd="Manger"),
  cor_short=structure(
    round(100*mapply(cor,data.table(y),data.table(z)),digits=1),
    format=quote(sign_color(n_fmt(this),this)),
    hdr=tbl(c("Realized","correlation","intraday","2 weeks"))
  ),
  cor_long=structure(
    round(100*mapply(cor,data.table(v),data.table(w)),digits=1),
    format=quote(sign_color(n_fmt(this),this)),
...
if(!off_site){
  ...
  log_code(duke<-fetch_bucket_pnl(db))
  log_code(res0<-mapply(
    function(m)pair_stats(paste0("^",m)),
    duke[,bucket]%>%stri_replace_all("",regex="_PAIR_[0-9]+$")%>%unique%>%sort,
    SIMPLIFY=FALSE
  ))
  log_code(res1<-mapply(function(x,n)paste0(
    "\\subsection{",n,"}\n",
    "\\vskip 5mm \n",
    ntable(df=x),
    "\\vskip 5mm \n"
