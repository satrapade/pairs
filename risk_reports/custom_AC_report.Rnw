...
the_pairs<-colnames(mc(pair_exposure,"AC[0-9]+"))
...
exposure_earnings<-do.call(rbind,res)[,c(
...
\noindent This report contains summary analytics and an event calendar for AC's pairs.
...
\newpage 
\section{Upcoming earnings announcements for AC's portfolio holdings}
...
\Sexpr{ntable(
  ...
  scale=0.75-max(length(the_pairs)-15,0)*0.07,
  alternating=FALSE,
  align=a1,
  halign="c",
  row_end="\\hline\\\\[-4mm]"
...
\newpage
\section{Pair drawdown vs return over the last 2 weeks.}
