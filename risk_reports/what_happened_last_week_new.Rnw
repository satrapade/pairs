...
x1<-NNcast(
  data=WHLW,
  i_name="manager_subsets",
  j_name="date",
  v_name="pnl"
) %>% 
  round(digits=0) %>%
  {data.table(subset=rownames(.),.)}

clean_contribution_names<-function(x){
  strsplit(x,"|",fixed=TRUE)[[1]][1]
}

x2<-NNcast(
  data=WHLW,
  i_name="contribution_subsets",
  j_name="date",
  v_name="pnl"
) %>% 
  round(digits=0) %>%
...
x3<-rbind(x1,x2)
x3[[1]]<-mapply(clean_contribution_names,x3[[1]])
for(i in 2:ncol(x3))x3[[i]]<-structure(x3[[i]],format=quote(n_fmt(this)))
