...
pair_cor_df<-data.table(
  ...
  daily=structure(daily_pair_cor,format=quote(sign_color(t=round(100*this,digits=2),s=this))),
  intraday=structure(intraday_pair_cor,format=quote(sign_color(t=round(100*this,digits=2),s=this)))
)[order(abs(intraday))]
...
\begin{tabular}{c c}
\Sexpr{ntable(
  df=pair_cor_df[order(abs(intraday))],
  title="ordered intraday pair correlation",
  scale="0.5"
)}
&
\Sexpr{ntable(
  df=pair_cor_df[order(abs(daily))],
  title="ordered daily pair correlation",
  scale="0.5"
)}
\\
\end{tabular}
