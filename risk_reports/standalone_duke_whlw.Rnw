...
performance_image<-function(
  ...
  {melt(data=.,id.vars="tick",measure.vars=rownames(intraday_duke_pairs),variable.factor=FALSE)} %>%
  {.[,.(
    tick=tick,
    variable=variable,
    value=value,
...
pair_rp<-duke_position_scrape[,.(
  ...
  list(
    daily_conviction=structure(
      ...
      hdr=tbl(c("daily","conviction")),
      format=quote(round(this,digits=2))
    ),
    intraday_conviction=structure(
      1/intraday_rp_gross_ratio,
      hdr=tbl(c("intraday","conviction")),
      format=quote(round(this,digits=2))
    )
  )
)]

# rp_duke_pairs<-duke_pairs[,pair_rp$pair]%*%diag(pair_rp$daily_rp_gross_ratio)
# rp_intraday_duke_pairs<-intraday_duke_pairs[,pair_rp$pair]%*%diag(pair_rp$intraday_rp_gross_ratio)
# 
# apply(rp_intraday_duke_pairs,2,sd)
# apply(rp_duke_pairs,2,sd)
...
\Sexpr{make_plot(plot(
  performance_image("^ABC[0-9]+$",txt_size=10)
),width="10cm",height="10cm",envir=environment())}
&
\Sexpr{make_plot(plot(
  performance_image("^ABC[0-9]+$",txt_size=10,xform_fun=function(x)100*cumsum(sign(x))/length(x))
),width="10cm",height="10cm",envir=environment())}
\\
\Sexpr{ntable(
  df=pair_rp[manager=="ABC",.(pair,daily_conviction,intraday_conviction)][order(-daily_conviction)]
)}
&
\\
\end{tabular}
...
Cummulative P\&L (bps) & Cummulative hit ratio (pct) \\
\Sexpr{make_plot(plot(
  performance_image("^MC[0-9]+$",txt_size=10)
),width="10cm",height="10cm",envir=environment())}
&
\Sexpr{make_plot(plot(
  performance_image("^MC[0-9]+$",txt_size=10,xform_fun=function(x)100*cumsum(sign(x))/length(x))
),width="10cm",height="10cm",envir=environment())}
\\
\Sexpr{ntable(
  df=pair_rp[manager=="MC",.(pair,daily_conviction,intraday_conviction)][order(-daily_conviction)]
)}
&
\\
\end{tabular}
...
Cummulative P\&L (bps) & Cummulative hit ratio (pct) \\
\Sexpr{make_plot(plot(
  performance_image("^DH[0-9]+$",txt_size=10)
),width="10cm",height="10cm",envir=environment())}
&
\Sexpr{make_plot(plot(
  performance_image("^DH[0-9]+$",txt_size=10,xform_fun=function(x)100*cumsum(sign(x))/length(x))
),width="10cm",height="10cm",envir=environment())}
\\
\Sexpr{ntable(
  df=pair_rp[manager=="DH",.(pair,daily_conviction,intraday_conviction)][order(-daily_conviction)]
)}
&
\\
\end{tabular}
...
Cummulative P\&L (bps) & Cummulative hit ratio (pct) \\
\Sexpr{make_plot(plot(
  performance_image("^JR[0-9]+$",txt_size=10)
),width="10cm",height="10cm",envir=environment())}
&
\Sexpr{make_plot(plot(
  performance_image("^JR[0-9]+$",txt_size=10,xform_fun=function(x)100*cumsum(sign(x))/length(x))
),width="10cm",height="10cm",envir=environment())}
\\
\Sexpr{ntable(
  df=pair_rp[manager=="JR",.(pair,daily_conviction,intraday_conviction)][order(-daily_conviction)]
)}
&
\\
\end{tabular}
...
Cummulative P\&L (bps) & Cummulative hit ratio (pct) \\
\Sexpr{make_plot(plot(
  performance_image("^AC[0-9]+$",txt_size=10)
),width="10cm",height="10cm",envir=environment())}
&
\Sexpr{make_plot(plot(
  performance_image("^AC[0-9]+$",txt_size=10,xform_fun=function(x)100*cumsum(sign(x))/length(x))
),width="10cm",height="10cm",envir=environment())}
\\
\Sexpr{ntable(
  df=pair_rp[manager=="AC",.(pair,daily_conviction,intraday_conviction)][order(-daily_conviction)]
)}
&
\\
\end{tabular}
...
Cummulative P\&L (bps) & Cummulative hit ratio (pct) \\
\Sexpr{make_plot(plot(
  performance_image("^GJ[0-9]+$",txt_size=10)
),width="10cm",height="10cm",envir=environment())}
&
\Sexpr{make_plot(plot(
  performance_image("^GJ[0-9]+$",txt_size=10,xform_fun=function(x)100*cumsum(sign(x))/length(x))
),width="10cm",height="10cm",envir=environment())}
\\
\Sexpr{ntable(
  df=pair_rp[manager=="GJ",.(pair,daily_conviction,intraday_conviction)][order(-daily_conviction)]
)}
&
\\
\end{tabular}
...
\newpage
\section{ACTW performance image}
...
Cummulative P\&L (bps) & Cummulative hit ratio (pct) \\
\Sexpr{make_plot(plot(
  performance_image("^ACTW[0-9]+$",txt_size=10)
),width="10cm",height="10cm",envir=environment())}
&
\Sexpr{make_plot(plot(
  performance_image("^ACTW[0-9]+$",txt_size=10,xform_fun=function(x)100*cumsum(sign(x))/length(x))
),width="10cm",height="10cm",envir=environment())}
\\
\Sexpr{ntable(
  df=pair_rp[manager=="ACTW",.(pair,daily_conviction,intraday_conviction)][order(-daily_conviction)]
)}
&
\\
\end{tabular}
