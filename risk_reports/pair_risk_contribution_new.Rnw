...
log_code(pair_stats$perf1m<-pair_stats[
  TRUE,
  structure(
    10000*colSums(tail(pair_pnl_matrix[,pair],30)),
    format=quote(cell_color(add_latex_env(paste0(ifelse(this>0,"\\phantom{-}",""),comma(this,digits=0)),"\\tt"),signed_ramp(this))),
    hdr=tbl(c("Perf","1m"),align="@{}l@{}") 
 )
])
  
log_code(pair_stats$perf1w<-pair_stats[
  TRUE,
  structure(
    10000*colSums(tail(pair_pnl_matrix[,pair],7)),
    format=quote(cell_color(add_latex_env(paste0(ifelse(this>0,"\\phantom{-}",""),comma(this,digits=0)),"\\tt"),signed_ramp(this))),
