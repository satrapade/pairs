...
pair_stats$marginal_risk_contribution_1y<-structure(
    mrc(total_gross*gross_fraction,cov(tail(pair_tret,365))),
    format=quote(n_fmt(round(this,digits=1))),
...
log_code(pair_stats$cum_marginal_risk_1y<-pair_stats[
  TRUE,
  structure(
...
log_code(pair_stats<-data.table(pair_stats,plt[match(pair_stats$pair,rownames(pair_lt_matrix))]))
...
log_code(pair_stats$perf3m<-pair_stats[
  TRUE,
  structure(
    10000*colSums(tail(pair_pnl_matrix[,pair],91)),
...
log_code(pair_stats$perf1m<-structure(
    10000*colSums(tail(pair_pnl_matrix[,pair_stats$pair],30)),
    format=quote(cell_color(add_latex_env(paste0(ifelse(this>0,"\\phantom{-}",""),comma(this,digits=0)),"\\tt"),signed_ramp(this))),
    hdr=tbl(c("Perf","1m"),align="@{}l@{}") 
))
  
log_code(pair_stats$perf1w<-structure(
    10000*colSums(tail(pair_pnl_matrix[,pair_stats$pair],7)),
    format=quote(cell_color(add_latex_env(paste0(ifelse(this>0,"\\phantom{-}",""),comma(this,digits=0)),"\\tt"),signed_ramp(this))),
...
log_code(pair_stats$hit1w<-structure(
    100*cumsum(pmax(sign(colSums(tail(pair_pnl_matrix[,pair_stats$pair],30))),0))/seq_along(pair_stats$pair),
